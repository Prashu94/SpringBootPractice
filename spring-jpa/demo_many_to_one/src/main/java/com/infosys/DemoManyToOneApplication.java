...
import com.infosys.service.CustomerLoanService;

@SpringBootApplication
public class DemoManyToOneApplication implements CommandLineRunner{
	private static final Log LOGGER = LogFactory.getLog(DemoManyToOneApplication.class);
	...
	public void addLoanAndCustomer() {
		try {
			...
			LOGGER.info("UserInterface.NEW_LOAN_CUSTOMER_SUCCESS: " +loanId);
		}catch(Exception e) {
			String message = environment.getProperty(e.getMessage(),"Some exception occured. Please check log file for more details!!");
			LOGGER.info(message);
		}
